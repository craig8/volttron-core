...
from volttron.types import AgentOptions
...
class Agent(AbstractAgent):
    ...
    def __init__(self, *, credentials: Credentials = None, options: AgentOptions = None, **kwargs):
        ...
        if identity is not None and not is_valid_identity(identity):
            ...
            _log.warning("All characters in {identity} are not in the valid set.".format(identity=identity))

        if options is None:
            options = AgentOptions()

        # TODO: We need to be able to get the address from environment probably here.
        context = AgentContext(credentials=credentials, address="tcp://127.0.0.1:22916", options=options)
